...
class Transaksi_langganan extends CI_Controller
{
    ...
    {
        $data['title'] = 'Transaksi langganan - Sistem Kasir Dg. Tiro';
        $data['produk'] = $this->TransaksiModel->tampil_data();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar_langganan');
        $this->load->view('templates/navbar', $data);
        $this->load->view('administrator/transaksi/langganan', $data);
    ...
    public function tambah($id)
    {
        ...
        $data = array(
            'id' => $this->input->post('sell_type', TRUE) . '-' . 'langganan',
            'produk_id' => $this->input->post('ID_produk', TRUE),
            'name' => $this->input->post('nama_produk', TRUE),
            'price' => $this->input->post('harga_langganan', TRUE),
        ...
        if ($qty > $produk['stok']) {
            ...
            $kurangi = $produk['stok'] - $qty;

            $this->db->update('tb_produk', array('stok' => $kurangi), array('id_produk' => $produk['id_produk']));
    ...
    function update_cart($produk_id)
    {
        ...
        $cart = $this->cart->get_item($this->input->post('row_id', TRUE));

        $qty_edit = $this->input->post('quantity', TRUE) - $cart['qty'];
    ...
    function hapus_cart($produk_id)
    {
        ...
        $restore = $produk['stok'] + $cart['qty'];

        $this->db->update('tb_produk', array('stok' => $restore), array('id_produk' => $produk['id_produk']));
    ...
    public function print_transaction($ID)
    {
        ...
        $this->load->view('administrator/transaksi/print_nota', $data);
    }
    ...
    public function cancel_transaction()
    {
        ...
        $this->cart->destroy();
        return redirect(site_url('administrator/transaksi_langganan'));
    }
}
