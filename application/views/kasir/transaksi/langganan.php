    ...
    <div class="agile-grids">
        ...
        <div class="grids">
            <div class="input-info">
                <h3 style="color:#00bcd4;">Transaksi langganan</h3>
            </div>
                                ...
                                <tr>
                                    <td style="width: 60%; ">
                                        <div style="text-align: center;">
                                            ...
                                            <a href="<?php echo site_url('kasir/transaksi_langganan/cancel_transaction') ?>" class="btn btn-danger"><i class="fa fa-times"></i> Batalkan</a>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" class="form-control" id="selling_type" name="selling_type" value="langganan">
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <table style="width: 90%; margin-left:auto; margin-right:auto">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td>
                                        <label style="margin-top: 10px;margin-bottom: 10px;">Jumlah Beli</label>
                                        <input type="number" id="qty_edit" name="qty" style="height: 40px; width: 150px; font-size: larger; background:#f4f7f9;" value="1" required="true" class="form-control">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('kasir/transaksi_langganan/data_transaksi') ?>",
                "type": "POST"
            },
        })

        table_order = $('#table_produk').DataTable({
            "info": false,
            "processing": true,
            "serverSide": true,
            "ordering": false,
            "ajax": {
                "url": "<?php echo site_url('kasir/transaksi_langganan/data_produk') ?>",
                "type": "POST"
            },
        })
    ...
    $.ajax({
        url: "<?php echo site_url('kasir/transaksi_langganan/get_total/') ?>",
        type: "GET",
        dataType: "JSON",
    ...
    $('#kode_barang').on('change', function() {
        if ($(this).val().length > 5) {
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_langganan/get_data/') ?>" + $(this).val(),
                type: 'GET',
                dataType: 'JSON',
    ...
    function add_cart_cari(id) {
        $.ajax({
            url: "<?php echo site_url('kasir/transaksi_langganan/get_data/') ?>" + id,
            type: 'GET',
            dataType: 'JSON',
            cache: false,
            success: function(obj) {
    ...
    function masukkan() {
        ...
        $.ajax({
            url: "<?php echo site_url('kasir/transaksi_langganan/tambah/') ?>" + id,
            type: 'POST',
            data: {
                sell_type: sell_type,
    ...
    function update_cart(id, produk_id) {
        $.ajax({
            url: "<?php echo site_url('kasir/transaksi_langganan/get_cart/') ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(obj) {
        ...
        $('#iya_edit').click(function() {
            ...
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_langganan/update_cart/') ?>" + produk_id,
                method: "POST",
                data: {
                    row_id: id,
                    quantity: quantity
    ...
    function hapus_cart(id, produk_id) {
        ...
        $('#iya_hapus').click(function() {
            ...
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_langganan/hapus_cart/') ?>" + produk_id,
                method: "POST",
                data: {
                    row_id: id
                },
    ...
    $('#dibayar').on('change keyup', function() {
        ...
        if ($(this).val().length >= 4) {
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_langganan/hitung/') ?>" + ($(this).val().replace(/[^\d]/g, '')),
                dataType: "JSON",
                beforeSend: function() {
    ...
    function button_selesai() {
        ...
        if (bayar == 0) {
            ...
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_langganan/tambah_transaksi') ?>",
                type: "POST",
                data: {
                    bayar: bayar
                },
    ...
    function popup(id) {
        newwindow = window.open("<?php echo site_url('kasir/transaksi_langganan/print_transaction/') ?>" + id + '?print=yes', 'name', 'height=600,width=800')

        if (window.focus) {
            newwindow.focus()
        }
        window.location.assign("<?php echo site_url('kasir/transaksi_langganan') ?>")
