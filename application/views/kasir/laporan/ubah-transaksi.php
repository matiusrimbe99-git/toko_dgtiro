                                ...
                                <tr>
                                    <td style="width: 60%; ">
                                        <div style="text-align: center;">
                                            <a href="<?php echo site_url('kasir/transaksi_umum/print_transaction/' . $transaksi->id_transaksi) ?>" target="_blank" class="btn btn-app btn-print btn-danger"><i class="fa fa-print"></i> Cetak Ulang Nota</a>
                                        </div>
                                    </td>
                                    <td style="width: 12%;">
                                        <h4>Tunai</h4>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#table-update-transaction').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('kasir/data_transaksi/getdataupdate/') ?>" + $('#table-update-transaction').data('id'),
                "type": "POST"
            },
        })
        ...
        table2 = $('#table_produk').DataTable({
            ...
            "ordering": false,
            "ajax": {
                "url": "<?php echo site_url('kasir/data_transaksi/data_produk/') ?>" + $('#table-update-transaction').data('selling'),
                "type": "POST"
            },
        })
    ...
    window.add_cart_cari = function(id) {
        if ($('#table-update-transaction').data('selling') == "umum") {
            $.ajax({
                url: "<?php echo site_url('kasir/transaksi_umum/get_data/') ?>" + id,
                type: 'GET',
                dataType: 'JSON',
                cache: false,
    ...
    $('#kode_barang').on('change', function() {
        if ($(this).val().length > 5) {
            if ($('#table-update-transaction').data('selling') == "umum") {
                $.ajax({
                    url: "<?php echo site_url('kasir/transaksi_umum/get_data/') ?>" + $(this).val(),
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(obj) {
                        if (obj.status == true) {
    ...
    function masukkan() {
        ...
        $.ajax({
            url: "<?php echo site_url('kasir/data_transaksi/addedcart/') ?>" + id,
            type: 'POST',
            data: {
    ...
    window.reload_table = function() {
        ...
        $.ajax({
            url: "<?php echo site_url('kasir/data_transaksi/hitung/') ?>" + ($('#dibayar').val().replace(/[^\d]/g, '')),
            method: "POST",
            data: {
                transaction: $('#table-update-transaction').data('id')
            },
    ...
    $.ajax({
        url: "<?php echo site_url('kasir/data_transaksi/get_total/') ?>",
        type: "POST",
        data: {
            transaction: $('#table-update-transaction').data('id')
        },
    ...
    $('#dibayar').on('change keyup', function() {
        ...
        if ($(this).val().length >= 4) {
            $.ajax({
                url: "<?php echo site_url('kasir/data_transaksi/hitung/') ?>" + ($(this).val().replace(/[^\d]/g, '')),
                method: "POST",
                data: {
                    transaction: $('#table-update-transaction').data('id')
                },
    ...
    function hapus_cart(produk, produk_id) {
        $.ajax({
            url: "<?php echo site_url('kasir/data_transaksi/get_cart/') ?>" + produk,
            type: 'GET',
            dataType: 'JSON',
            cache: false,
            success: function(obj) {
        ...
        $('#iya_hapus').click(function() {
            ...
            $.ajax({
                url: "<?php echo site_url('kasir/data_transaksi/delete_item/') ?>" + produk,
                method: "GET",
                dataType: "JSON",
                success: function() {
                    $('#modal-hapus').modal('hide')
